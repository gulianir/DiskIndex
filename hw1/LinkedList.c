...
bool SliceLinkedList(LinkedList list, LLPayload_t *payload_ptr) {
  ...
  *payload_ptr = list->tail->payload;
  if (list->num_elements == 1) {
    ...
    LinkedListNodePtr new_last = list->tail->prev;
    new_last->next = NULL;
    free(list->tail);
    list->tail = new_last;
  }
...
bool LLIteratorDelete(LLIter iter,
  ...
  LinkedListNodePtr to_delete = iter->node;

  if (iter->list->num_elements == 1) {
    // node's next and prev should already be null w/ just one element
    // but defensive programming:
    iter->node->next = iter->node->prev = NULL;
