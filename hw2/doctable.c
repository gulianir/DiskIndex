...
DocID_t DTRegisterDocumentName(DocTable table, char *docname) {
  ...
  *docid = table->max_id;
  ...
  res = InsertHashTable(table->docid_to_docname, kv, &oldkv) != 0;
  Verify333(res != 0);
  ...
  kv.key = (HTKey_t) FNVHash64((unsigned char*) doccopy, strlen(doccopy));
...
DocID_t DTLookupDocumentName(DocTable table, char *docname) {
  ...
  if (res == 1) {
    DocID_t* valPtr = (DocID_t*) kv.value;
    return *valPtr;
  }
  return 0;
}
