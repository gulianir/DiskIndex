...
int MIAddPostingList(MemIndex index, char *word, DocID_t docid,
  ...
  if (res == 0) {
    ...
    Verify333(wds != NULL);
    wds->word = word;
    // size arbitrary, will grow dynamically
    HashTable ht = AllocateHashTable(2);
    Verify333(ht != NULL);
    wds->docIDs = ht;
    kv.key = (HTKey_t) wordkey;
    kv.value = (HTValue_t) wds;
    Verify333(InsertHashTable(index, kv, &hitkv) != 0);
  } else {
...
LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  ...
  while(!HTIteratorPastEnd(it)) {
    ...
    Verify333(res != 0);
    SearchResultPtr searchRes = (SearchResultPtr) malloc(sizeof(SearchResult));
    Verify333(searchRes != NULL);
    searchRes->docid = kv.key;
    searchRes->rank = NumElementsInLinkedList((LinkedList) kv.value);
    bool appendRes = AppendLinkedList(retlist, (LLPayload_t) searchRes);
    Verify333(appendRes);
  ...
  for (i = 1; i < qlen; i++) {
    ...
    wds = (WordDocSetPtr) kv.value;
    HashTable newTable = (HashTable) wds->docIDs;
    for (j = 0; j < ne; j++) {
      SearchResultPtr searchRes;
      LLIteratorGetPayload(llit, (LLPayload_t*)&searchRes);
      res = LookupHashTable(newTable, searchRes->docid, &kv);
      Verify333(res != -1);
      if (res == 1) {
        searchRes->rank += NumElementsInLinkedList(kv.value);
        LLIteratorNext(llit);
      } else {
