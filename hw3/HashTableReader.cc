...
HashTableReader::LookupElementPositions(HTKey_t hashKey) {
  ...
  // MISSING:
  IndexFileOffset_t bOffset = b_rec.bucket_position;
  element_position_rec elPos;

  res = fseek(file_, bOffset, SEEK_SET);
  Verify333(res == 0);

  for (HWSize_t numElements = b_rec.chain_len; numElements > 0;
       numElements--) {
    res = fread(&elPos, sizeof(elPos), 1, file_);
    Verify333(res == 1);
    elPos.toHostFormat();

    retval.push_back(elPos.element_position);
  }
