...
class QueryProcessor {
 ...
 private:
  ...
  // Helper method takes in a single word and returns a vector
  // of QueryResults.
  void GetResults(const string& word, vector<QueryResult>& results);
  // Helper method takes in two vector and removes from the first vector
  // any QueryElement elements not in the intersection of the two vectors
  // (with the same name). Updates the ranks of the QueryResults in the
  // finalresult of elements found in curr.
  void SubsetFromFinal(vector<QueryResult>& finalresult, 
