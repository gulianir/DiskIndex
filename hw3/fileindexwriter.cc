...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  // MISSING:
  docid_element_header_st docHeader;
  docHeader.docid = docID_ho;
  docHeader.num_positions = num_pos_ho;

  docHeader.toDiskFormat();

  res = fseek(f, offset, SEEK_SET);
  if (res != 0) {
    return 0;
  }
  res = fwrite(&docHeader, sizeof(docHeader), 1, f);
  if (res != 1) {
    return 0;
  }
  ...
  return sizeof(docHeader) + num_pos_ho * sizeof(position);
}
...
static HWSize_t WriteHeader(FILE *f,
  ...
  char* buf = new char[cslen];
  res = fread(buf, cslen, 1, f);
  if (res != 1) {
    return 0;
  }

  for (HWSize_t i = 0; i < cslen; i++) {
    crcobj.FoldByteIntoCRC((uint8_t)buf[i]);
  }
...
static HWSize_t WriteBucketRecord(FILE *f,
  ...
  // MISSING:
  int write = fwrite(&br, sizeof(bucket_rec), 1, f);
  if (write != 1) {
    return 0;
  }
...
static HWSize_t WriteBucket(FILE *f,
  ...
  if (chainlen_ho > 0) {
    ...
    for (j = 0; j < chainlen_ho; j++) {
      ...
      // MISSING:
      element_position_rec elPos;
      elPos.element_position = nextelpos;
      elPos.toDiskFormat();

      res = fseek(f, offset + j*sizeof(elPos), SEEK_SET);
      if (res != 0) {
        LLIteratorFree(it);
        return 0;
      }
...
static HWSize_t WriteHashTable(FILE *f,
  ...
  for (i = 0; i < ht->num_buckets; i++) {
    // MISSING:
    HWSize_t writeBucketRec = WriteBucketRecord(f,
                                                ht->buckets[i],
    ...
    HWSize_t writeBucket = WriteBucket(f,
                                       ht->buckets[i],
                                       next_bucket_offset,
                                       fn);
    if (writeBucket == 0) {
    ...
    next_bucket_offset += writeBucket;
  }
